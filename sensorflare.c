...
void sendMessage(const char * messagebody) {
    if (!sensorflare_connected) return;
...
void init_sensorflare(void) {
    ...
    if (1 == 1) {
	pthread_t receiver_thread, status_thread;
	long t = 0;
	int receiver_rc = pthread_create(&receiver_thread, NULL, receiver, (void *) t);
	if (receiver_rc) {
	    syslog(LOG_ERR, "return code from pthread_create() is %d\n", receiver_rc);
	    exit(-1);
	}
	int status_rc = pthread_create(&status_thread, NULL, status_reporting, (void *) t);
	if (status_rc) {
	    syslog(LOG_ERR, "return code from pthread_create() is %d\n", status_rc);
	    exit(-1);
	}
